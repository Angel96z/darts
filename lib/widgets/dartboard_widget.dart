...
class _DartboardWidgetState extends State<DartboardWidget> {
  ...
  DartHitData _calculateHit({
    ...
    final center = Offset(boardRadius, boardRadius);
    ...
    final base = _sectors[sectorIndex];
    ...
    // ===== BULL MODE =====
    if (widget.target != null && widget.target!.endsWith("25")) {

      final ddx = boardPosition.dx - center.dx;
      final ddy = boardPosition.dy - center.dy;

      final pixelDistance = sqrt(ddx * ddx + ddy * ddy);

      distanceMm = (pixelDistance / boardRadius) * 225.5;

      if (sector == widget.target) {
        targetQuadrant = "center";
      } else {
        if (ddx < 0 && ddy < 0) targetQuadrant = "tl";
        if (ddx >= 0 && ddy < 0) targetQuadrant = "tr";
        if (ddx < 0 && ddy >= 0) targetQuadrant = "bl";
        if (ddx >= 0 && ddy >= 0) targetQuadrant = "br";
      }
    ...
    if (widget.target != null) {
      ...
      final ring = widget.target![0];
      final value = int.tryParse(widget.target!.substring(1));
      ...
      if (value != null) {
        ...
        if (index != -1) {
          ...
          final targetX = center.dx + cos(angle) * targetRadius;
          final targetY = center.dy + sin(angle) * targetRadius;

          final ddx = boardPosition.dx - targetX;
          final ddy = boardPosition.dy - targetY;
